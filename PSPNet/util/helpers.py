...
def overlay_mask(im, ma, color=np.array([255, 0, 0])/255.0):
    ...
    hand = (ma == 1)
    hand = hand.astype(np.bool)
    obj = (ma == 2)
    obj = obj.astype(np.bool)
    ...
    fg2= im * alpha + np.ones(im.shape) * (1 - alpha) * np.array([0, 255, 0])/255.0
    ...
    contours = cv2.findContours(hand.copy().astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
    cv2.drawContours(bg, contours[0], -1, (0.0, 0.0, 0.0), 1)
    contours2 = cv2.findContours(obj.copy().astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
    cv2.drawContours(bg, contours2[0], -1, (0.0, 0.0, 0.0), 1)
    return bg
